...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/fee-dues", async (req: Request, res: Response) => {
    try {
      const validatedData = insertFeeDueSchema.parse(req.body);
      const feeDue = await storage.createFeeDue(validatedData);
      res.status(201).json(feeDue);
    } catch (error) {
      if (error instanceof z.ZodError) {
        res.status(400).json({ message: "Validation error", errors: error.errors });
      } else if (error instanceof Error) {
      ...
      for (const row of data) {
        try {
          ...
          const student = {
            ...
            rollNumber: row['Roll No.'] || row['RollNumber'] || 0,
            parentName: row['Parent Name'] || row['ParentName'] || '',
            contactNumber: row['Contact No.'] || row['ContactNumber'] || '',
            email: row['Email'] || '',
            feeCategory: row['Fee Category'] || row['FeeCategory'] || 'Regular',
