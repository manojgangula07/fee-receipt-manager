...
export function Sidebar({ isOpen, onToggle }: SidebarProps) {
  ...
  return (
    <div
      className={cn(
        "fixed inset-y-0 left-0 z-30 w-64 bg-white border-r border-gray-200 shadow-md text-gray-800 transition-transform duration-300",
...
function SidebarContent({ currentPath }: SidebarContentProps) {
  ...
  const schoolInitials = settings?.schoolName 
    ? settings.schoolName.split(' ').map(word => word[0]).join('').slice(0, 2) 
    : "SF";
        ...
        <div className="flex items-center space-x-3">
          {settings?.logo ? (
            <Avatar className="h-10 w-10">
              <AvatarImage src={settings.logo} alt={schoolName} />
          ...
          <div className="font-semibold text-gray-800 truncate">{schoolName}</div>
        </div>
      </div>
