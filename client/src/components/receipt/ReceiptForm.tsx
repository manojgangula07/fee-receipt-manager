...
export function ReceiptForm({ onReceiptGenerated }: ReceiptFormProps) {
  ...
  const { control, handleSubmit, setValue, watch, formState: { errors } } = useForm<z.infer<typeof paymentSchema>>({
  ...
  const showReferenceField = paymentMethod === 'Check' || paymentMethod === 'Online Transfer' || paymentMethod === 'UPI' || paymentMethod === 'Credit/Debit Card';

  // Query to search for students
  const { data: students, isLoading: isSearching, refetch } = useQuery({
  ...
  const generateReceiptNumber = async (grade: string, section: string) => {
    try {
      const response = await apiRequest('GET', `/api/generate-receipt-number?grade=${grade}&section=${section}`, undefined);
  ...
  const totalAmount = selectedFees.reduce((sum, fee) => sum + (fee.amount - fee.amountPaid), 0);
      ...
      {step === 'search' ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Search for a student and generate a fee receipt.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="mb-6 flex flex-col space-y-4 sm:flex-row sm:space-x-4 sm:space-y-0">
              ...
              <div className="w-full sm:w-auto">
                <Select
                  ...
                  onValueChange={(value) => setSearchParams({ ...searchParams, grade: value })}
                >
                  ...
                  <SelectContent>
                    ...
                    {GRADES.map((grade) => (
                      <SelectItem key={grade} value={grade}>{grade === 'KG' ? 'Kindergarten' : `Class ${grade}`}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            ...
            {students && students.length > 0 ? (
              <div className="mt-4 border rounded-md">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th scope="col" className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Admission No.</th>
                      <th scope="col" className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Student Name</th>
                      <th scope="col" className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Class</th>
                      <th scope="col" className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Parent Name</th>
                    ...
                    {students.map((student: any) => (
                      <tr key={student.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <Button size="sm" variant="outline" onClick={() => handleSelectStudent(student)}>
                            Select
                          </Button>
                        </td>
                      </tr>
              ...
              <div className="mt-4 p-4 text-center bg-gray-50 rounded-md">
                <p className="text-gray-600">No students found. Try a different search term.</p>
              </div>
            ) : null}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Create a receipt for <span className="font-medium">{selectedStudent?.studentName}</span> ({selectedStudent?.grade}-{selectedStudent?.section})
            </CardDescription>
          </CardHeader>
          <CardContent>
            {/* Student Details */}
            <div className="mb-6 grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2 md:grid-cols-3">
              <div>
                ...
                <Input type="text" value={selectedStudent?.admissionNumber} readOnly className="bg-gray-100" />
              </div>
              <div>
                <Label>Student Name</Label>
                <Input type="text" value={selectedStudent?.studentName} readOnly className="bg-gray-100" />
              </div>
              <div>
                <Label>Class & Section</Label>
                <Input type="text" value={`${selectedStudent?.grade}-${selectedStudent?.section}`} readOnly className="bg-gray-100" />
              </div>
              <div>
                <Label>Parent Name</Label>
                <Input type="text" value={selectedStudent?.parentName} readOnly className="bg-gray-100" />
              </div>
              <div>
                <Label>Contact Number</Label>
                <Input type="text" value={selectedStudent?.contactNumber} readOnly className="bg-gray-100" />
              </div>
              <div>
                <Label>Fee Category</Label>
                <Input type="text" value={selectedStudent?.feeCategory} readOnly className="bg-gray-100" />
              </div>
            </div>
            ...
            <div className="overflow-hidden rounded-lg border border-gray-200">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Fee Type</th>
                    ...
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Select</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200 bg-white">
                  {isLoadingFeeDues ? (
                    ...
                    feeDues.map((feeDue) => (
                      <tr key={feeDue.id}>
                        <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-900">{feeDue.description}</td>
                        <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-900">
                          {(feeDue.amount - feeDue.amountPaid).toFixed(2)}
                        </td>
                        ...
                        <td className="whitespace-nowrap px-6 py-4 text-sm">
                          <Checkbox
                            checked={selectedFees.some(f => f.id === feeDue.id)}
                  ...
                  <tr className="bg-gray-50 font-medium">
                    <td className="whitespace-nowrap px-6 py-4 text-right text-sm text-gray-900">Total Selected:</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-900">
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-sm">
                      <Button variant="link" className="h-auto p-0 text-xs" onClick={handleSelectAllFees}>
                        {selectedFees.length === feeDues.length ? 'Deselect All' : 'Select All'}
                      </Button>
                    </td>
                  </tr>
                </tbody>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="mt-6">
                <h4 className="mb-4 font-medium text-gray-700">Payment Details</h4>
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
                  <div>
                    ...
                    />
                    {errors.receiptDate && (
                      <p className="mt-1 text-xs text-red-500">{errors.receiptDate.message}</p>
                    )}
                  </div>

                  <div>
                    <Label>Receipt Number</Label>
                    <Input value={receiptNumber} readOnly className="bg-gray-100" />
                  </div>
                  ...
                  <div>
                    ...
                    {errors.amountReceived && (
                      <p className="mt-1 text-xs text-red-500">{errors.amountReceived.message}</p>
                    )}
                  </div>
